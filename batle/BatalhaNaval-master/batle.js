...
function showGrid(ispc) {
  ...
  for (y=0;y<gridy;++y) {
    for (x=0;x<gridx;++x) {
      if ( ispc )
      document.write ('<a href="javascript:gridClick('+y+','+x+');"><img name="pc'+y+'_'+x+'" src="batt100.gif" width=50 height=50></a>');
      else
      document.write ('<a href="javascript:void(0);"><img name="ply'+y+'_'+x+'" src="batt'+player[y][x][0]+'.gif" width=50 height=50></a>');
...
function computerMove() {
  ...
  if (player[sy][sx][0] < 100) {
    ...
    if ( --playersships[shipno][1] == 0 ) {
      ...
      alert("Cpu afundou seu "+shiptypes[playersships[shipno][0]][0]+"!");
      if ( --playerlives == 0 ) {
        knowYourEnemy();
        alert("Cpu venceu! reinicie\n"+
        "no seu navegador para jogar novamente.");
        playflag = false;
      }
    }
